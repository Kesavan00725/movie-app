/* ================================================================
   CINEVERSE — watchlist.js
   Fetches and displays the logged-in user's watchlist from the
   FastAPI backend. Mirrors favorites.js structure exactly.
================================================================ */

'use strict';

const WL_API_BASE = 'https://movie-app-qhzc.onrender.com';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    hideSkeleton();
    showToast('Could not load watchlist. Please try again.', 'error');
    showState('wl-empty');
  }
});
...
async function fetchWatchlist() {
  const res = await fetch(`${WL_API_BASE}/watchlist/`, {
    headers: {
  ...
  for (const item of list) {
    if (item.title) {
      ...
      if (movieId) {
        try {
          const mRes = await fetch(`${WL_API_BASE}/movies/${movieId}`, {
            headers: { 'Accept': 'application/json', 'Authorization': `Bearer ${wlToken()}` }
          });
          if (mRes.ok) movies.push(normalizeMovie(await mRes.json()));
        } catch {}
      }
    }
  }
...
function resolveImg(raw) {
  ...
  if (raw.startsWith('/')) return WL_API_BASE + raw;
  return raw;
}
...
function renderWatchlist(movies) {
  ...
  grid.querySelectorAll('.fwl-wl-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      await removeFromWatchlist(movieId, btn.closest('.mgc'));
    });
  });
  ...
  grid.querySelectorAll('.fwl-fav-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        const res = await fetch(`${WL_API_BASE}/favorites/${movieId}`, {
          method,
          headers: { 'Accept': 'application/json', 'Authorization': `Bearer ${wlToken()}` }
        });
        if (res.ok || res.status === 404) {
          ...
          showToast(!active ? '♥ Added to Favorites' : 'Removed from Favorites', !active ? 'success' : '');
        }
      } catch {
...
async function removeFromWatchlist(movieId, cardEl) {
  try {
    const res = await fetch(`${WL_API_BASE}/watchlist/${movieId}`, {
      method: 'DELETE',
      headers: { 'Accept': 'application/json', 'Authorization': `Bearer ${wlToken()}` }
    });
    ...
    if (res.ok || res.status === 404) {
      if (cardEl) {
        ...
        setTimeout(() => {
          cardEl.remove();
          const remaining = document.querySelectorAll('#wl-grid .mgc').length;
          const count = document.getElementById('wl-count');
          if (count) count.textContent = `${remaining} film${remaining !== 1 ? 's' : ''}`;
          if (remaining === 0) showState('wl-empty');
        }, 320);
      }
      ...
      if (cardEl) {
        const btn = cardEl.querySelector('.fwl-wl-btn');
    ...
    <div class="mgc" role="listitem" style="animation: fwlFadeUp 0.5s cubic-bezier(0.16,1,0.3,1) ${i * 0.05}s both;">
      <div class="mgc-poster">
        <img src="${escHtml(m.poster_url)}" alt="${escHtml(m.title)}" loading="lazy"
             onerror="this.src='${WL_PLACEHOLDER}'">
        ...
        <div class="mgc-actions">
          ...
          <div class="mgc-action-row">
            <button class="mgc-action-btn fwl-wl-btn active" data-id="${m.id}" aria-label="Remove from Watchlist" title="Remove from Watchlist">
              ${bookmarkSvg(true)}
            </button>
            <button class="mgc-action-btn fwl-fav-btn" data-id="${m.id}" aria-label="Add to Favorites" title="Add to Favorites">
              ${heartSvg(false)}
            </button>
          </div>
        </div>
